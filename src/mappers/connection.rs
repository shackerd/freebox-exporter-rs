...
impl<'a> ConnectionMetricMap<'a> {
    pub fn new(factory: &'a AuthenticatedHttpClientFactory<'a>, prefix: String) -> Self {
        Self {
            ...
            .expect(&format!(
                "cannot {prefix}_create connection_ipv6_conf_ipv6_enabled"
            )),
            ...
            .expect(&format!(
                "cannot {prefix}_create connection_ipv6_conf_delegations"
            )),
...
impl<'a> MetricMap<'a> for ConnectionMetricMap<'a> {
    ...
    async fn set(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Err(e) = self.set_connection_conf().await {
            return Err(e);
        }
        if let Err(e) = self.set_connection_ipv6_conf().await {
            return Err(e);
        }
        if let Err(e) = self.set_connection_ftth().await {
            return Err(e);
        }
        Ok(())
    }
...
impl DryRunnable for ConnectionMetricMap<'_> {
    ...
    async fn dry_run(&mut self) -> Result<String,Box<dyn std::error::Error>>{
        ...
        Ok("\"connection result\"".to_string())
    }
